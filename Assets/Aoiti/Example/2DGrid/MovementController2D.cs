...
//    private void Update()
//    {
//        if (Input.GetMouseButtonDown(0)) //check for a new directionTarget
//        {
//            Vector3 directionTarget = Camera.main.ScreenToWorldPoint(Input.mousePosition);
//            var _path = new Vector3[0];
//            if (pathfinder.GenerateAstarPath(transform.position, directionTarget, out _path)) //if there is a path from current position to directionTarget position reassign path.
...
public class MovementController2D : MonoBehaviour
{
    ...
    [SerializeField] protected float speed = 0.05f; //increase for faster movement
    ...
    [SerializeField] protected bool searchShortcut =false; 
    [Tooltip("Deactivate to make the navigator to stop at the nearest point on the grid.")]
    [SerializeField] protected bool snapToGrid =false; 
    Vector2 targetNode; //directionTarget in 2D space
    protected List <Vector2> path;
    protected List<Vector2> pathLeftToGo= new List<Vector2>();
    ...
    protected virtual void Update()
    {
        ...
        if (pathLeftToGo.Count > 0) //if the directionTarget is not yet reached
        {
            Vector3 direction = (Vector3)pathLeftToGo[0] - transform.position;
    ...
    protected void GetMoveCommand(Vector2 target)
    {
        Vector2 closestNode = GetClosestNode(transform.position);
        if (pathfinder.GenerateAstarPath(closestNode, GetClosestNode(target), out path)) //Generate path between two points on grid that are close to the transform position and the assigned directionTarget.
        {
